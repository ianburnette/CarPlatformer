...
public class PlayerMove : MonoBehaviour 
{
	...
	//movement
	public float chargingAccel, chargingMaxSpeed;
	public float baseAccel = 70f, baseMaxSpeed = 9;
	...
	//jumping
	public float baseJumpForce1, baseJumpForce2, baseJumpForce3, chargeJumpForce;
	...
	public float fallGravity, glideGravity, glideBoost, maxGlideVerticalSpeed;
	public bool gliding;
	...
	{	
		...
		if (Input.GetButton ("Charge") && !gliding) {
			charging = true;
		} else {
			charging = false;
		}
		if (charging) {
			accel = chargingAccel;
			maxSpeed = chargingMaxSpeed;
			forwardOverride = 1;
			jumpForce = new Vector3 (0, chargeJumpForce, 0);
			secondJumpForce = new Vector3 (0, chargeJumpForce, 0);
			thirdJumpForce = new Vector3 (0, chargeJumpForce, 0);
		} else {
			accel = baseAccel;
			maxSpeed = baseMaxSpeed;
			if (!gliding)
				forwardOverride = 0;
			jumpForce = new Vector3 (0, baseJumpForce1, 0);
			secondJumpForce = new Vector3 (0, baseJumpForce2, 0);
			thirdJumpForce = new Vector3 (0, baseJumpForce3, 0);
		}
		...
		//apply forward movement override, if gliding or charging
	...
	void FixedUpdate() 
	{
		...
		grounded = IsGrounded ();
		if (!gliding) {
			if (anim.GetBool("wingsOut") == true){
				anim.SetBool("wingsOut", false);
			}
		}
		if (grounded) {
			if (gliding){
				gliding = false;
				anim.SetBool ("wingsOut", true);
			}
		}
...
//			print (rb.velocity.y);
	...
	private bool IsGrounded() 
	{
		...
		{
			RaycastHit hit;
			if(Physics.Raycast(check.position, Vector3.down, out hit, dist + 0.05f, groundMask))
			{
	...
	void GlidePhysics(){
		...
		Vector3 newVel = transform.forward * glideBoost;
		rb.velocity = new Vector3 (rb.velocity.x, rb.velocity.y, newVel.z);
	}
