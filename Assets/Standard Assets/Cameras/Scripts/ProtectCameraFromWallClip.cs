...
namespace UnityStandardAssets.Cameras
{
    public class ProtectCameraFromWallClip : MonoBehaviour
    {
		...
		public float distFromGround, minDist;
		...
		void CheckBelow(){
			RaycastHit hitInfo;
			if (Physics.Raycast (m_Cam.position + (Vector3.up * 0.1f), Vector3.down, out hitInfo, distFromGround, groundMask)) {
				if (hitInfo.distance < minDist){
					m_Cam.localPosition = new Vector3 (m_Cam.localPosition.x, Mathf.Lerp (m_Cam.localPosition.y, m_Cam.localPosition.y + minDist, clipMoveTime), m_Cam.localPosition.z);
				}
				//print (hitInfo.distance + " from " + hitInfo.transform);
			}
		}
